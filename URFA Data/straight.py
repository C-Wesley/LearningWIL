...
if __name__ == '__main__':
    ...
    s_dels = []
    paths = [f for f in os.listdir('./straight') if f.endswith(".csv") and not f.startswith(('path', 'GPSDATA'))]
    rms_values = []
    ...
    for (number, f) in enumerate(paths):
        dels = plotPath(os.path.join(curDir, f), os.path.join(curDir, 'path.csv'), xlim, ylim, f"Canal", plot)
        s_dels = s_dels + dels
    ...
    plt.title("Canal Path Deviation",fontsize=22)
