...
if __name__ == '__main__':
    ...
    trap_dels   = []
    elipse_dels = []
    square_paths = [f for f in os.listdir('./Data2/square') if f.endswith(".csv") and not f.startswith(('path', 'GPSDATA'))]
    elipse_paths = [f for f in os.listdir('./Data2/elipse') if f.endswith(".csv") and not f.startswith(('path', 'GPSDATA'))]
    circle_paths = [f for f in os.listdir('./Data2/circle') if f.endswith(".csv") and not f.startswith(('path', 'GPSDATA'))]
    trap_paths   = [f for f in os.listdir('./Data2/trap') if f.endswith(".csv") and not f.startswith(('path', 'GPSDATA'))]
